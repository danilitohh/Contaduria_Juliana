...
export const moduleRegistry: Record<ModuleKey, ModuleDefinition> = {
  clientes: {
    ...
    icon: UsersRound,
    rows: clients,
    searchKeys: ["nombre", "numero_documento", "email", "ciudad"],
    filters: [
  ...
  proveedores: {
    ...
    metrics: [
      ...
      { label: "Cuentas", value: formatCurrency(suppliers.reduce((sum, row) => sum + row.saldo_pendiente, 0)), helper: "Saldo pendiente" },
      { label: "Compras demo", value: String(expenses.length), helper: "Gastos relacionados" },
    ],
  },
  productos: {
    key: "productos",
